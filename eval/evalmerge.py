...
# Load base model
base_model_name = "allenai/OLMo-2-1124-7B-Instruct"  #  "mistralai/Mistral-7B-Instruct-v0.3"
base_model = AutoModelForCausalLM.from_pretrained(base_model_name, torch_dtype=torch.float16, device_map="auto")

# Load LoRA adapter
lora_model_path = "/data1/s2269651/fine_tune/O7B/lora_model_olmo7b_reason_baba"  # Change this to your actual LoRA path
...
Let's first understand the problem, extract the relevant objects, text blocks and rules, explain the rules in the level and make a plan to solve the problem. Then let's carry out the plan by giving the intermediate actions (using common sense). Solve the problem step by step and show the solution.""".strip()
    ...
    with torch.no_grad():
        output = merged_model.generate(**inputs, max_new_tokens=1024, temperature=0.5, top_p=0.7, do_sample=True)
