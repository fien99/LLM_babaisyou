...
def formatting_prompts_func4(example):
    output_texts = []
    for i in range(len(example['question'])):
...
#dataset4=load_dataset("thesven/gsm8k-reasoning",split="train")
...
#formatted_dataset4 = formatting_prompts_func4(dataset4)
# Combine datasets
combined_dataset = formatted_dataset1 + formatted_dataset2 + formatted_dataset3 #+ formatted_dataset4
...
model_id = "allenai/OLMo-2-1124-7B-Instruct"#"mistralai/Mistral-7B-Instruct-v0.3"  
tokenizer = AutoTokenizer.from_pretrained(model_id)
tokenizer.pad_token = tokenizer.eos_token
tokenizer.padding_side = "right"   

print(tokenizer(combined_dataset[0], truncation=True, padding="longest", return_tensors="pt"))
...
args=TrainingArguments(
        ...
        num_train_epochs=8, # replace this, depending on your dataset
    ...
    per_device_train_batch_size=4,  # Increase batch size for better efficiency
    gradient_accumulation_steps=16,  # Adjusted to avoid slowing down training
    num_train_epochs=5,Reasoning:\n {example['generation'][i]}\n Answer:\n {example['answer'][i]} # More warmup for stable early training
    ...
    bf16=True,  # Best for A100 (more stable than FP16)
    seed=42,    
    output_dir="mistral_c7b",
...
args = TrainingArguments(
    ...
    logging_dir="./logs",                # Directory for logs# Learning rate for fine-tuning
    #per_device_train_batch_size=8,      # Batch size per device
    auto_find_batch_size=True,
    gradient_accumulation_steps=6,       # Accumulate gradients to simulate larger batches
    ...
    warmup_steps=500,    # Reduce warmup to adapt to smaller LR
    max_grad_norm=1.0, 
    lr_scheduler_type="cosine"  # Clipping to prevent gradient explosion
)
...
trainer.train()
peft_model= os.path.join('O7', f"lora_model_olmo7b_reason_baba")
